#!/usr/bin/env python3
"""Build a dashboard snapshot from explicitly configured runtime roots."""
...
def _encoded_media_url(path: Path, root: Path, route_prefix: str) -> str | None:
    """Return a segment-encoded URL only when *path* resolves below *root*."""
    try:
        relative = path.resolve(strict=True).relative_to(root.resolve(strict=True))
    except (OSError, RuntimeError, ValueError):
        return None
    encoded = "/".join(quote(segment, safe="") for segment in relative.parts)
    return f"{route_prefix}/{encoded}" if encoded else None
...
def build_dashboard_snapshot(
    *,
    ...
    queue_pause_flag_path: Path,
    library_root: Path,
    preview_root: Path,
    report_output_root: Path,
) -> tuple[dict[str, Any], Path]:
    """Build and write ``_snapshot.json`` without inferring live paths."""
    COLLECTION_ROOT = collection_root
    STATE = queue_state_path
    PAUSE_FLAG = queue_pause_flag_path
    WORKER_LOG = COLLECTION_ROOT / ".wallpaper-download-queue" / "worker.log"
    LIBRARY = library_root
    TEMP_ROOT = preview_root
    TEMP_AP = TEMP_ROOT / "anime-pictures"
    TEMP_APF = TEMP_ROOT / "anime-pictures-full"
    TEMP_ZC = TEMP_ROOT / "zerochan"
    TEMP_WH = TEMP_ROOT / "wallhaven"
    TEMP_GB = TEMP_ROOT / "gelbooru"
    MOVEDIR = COLLECTION_ROOT / "movedir"
    ...
    def collect_preview_gallery(limit=160, per_source_cap=80, buffer_window_days=14, min_per_source=24):
        ...
        Snapshot entries contain only a media URL and display metadata. Absolute
        filesystem paths never leave this builder.
        """
        import os as _os
        ...
        def _add_file(fp, source_hint=None):
            ...
            route_root = LIBRARY if source_hint == "library" else TEMP_ROOT
            route_prefix = "/media/library" if source_hint == "library" else "/media/preview"
            media_url = _encoded_media_url(fp, route_root, route_prefix)
            ...
            sidecar = fp.with_name(f"{fp.stem}.wallpaper.json")
            ...
            items.append({
                "_path_key": str(fp.resolve(strict=False)),
                "media_url": media_url,
                "name": fp.name,
        ...
        for it in sorted(items, key=lambda x: x["mtime"], reverse=True):
            if it["_path_key"] in seen:
                continue
            seen.add(it["_path_key"])
            uniq.append(it)
        ...
        for source in sorted(by_source):
            for it in by_source[source][:reserve]:
                selected.append(it)
                selected_paths.add(it["_path_key"])

        for it in uniq:
            if len(selected) >= limit:
                break
            if it["_path_key"] in selected_paths:
                continue
            selected.append(it)
            selected_paths.add(it["_path_key"])

        output = sorted(selected[:limit], key=lambda x: x["mtime"], reverse=True)
        for item in output:
            item.pop("_path_key", None)
        return output
    ...
    snapshot["worker_paused"] = PAUSE_FLAG.exists()



    report_output_root.mkdir(parents=True, exist_ok=True)
    snap_path = report_output_root / "_snapshot.json"
...
def _argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build the operations snapshot from explicit runtime roots."
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    build_dashboard_snapshot(
        ...
        queue_pause_flag_path=args.pause_flag_path,
